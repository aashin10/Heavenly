...
- **Task 1 must land before Task 9** (which reads `myBid`) and **before Task 8** (which reads `canWithdraw`). Everything else is strictly sequential — each frontend task compiles against the one before it. Do not parallelise 2→9.
