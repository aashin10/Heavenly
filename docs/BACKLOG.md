...
**Last updated:** 2026-08-04 · **Maintained on the go** — updated after every coding iteration, not retroactively.
...
| Tests | **214 backend integration tests**, Testcontainers-backed, self-contained. **22 frontend specs** (interceptor, token-refresh coordinator, `legalVendorActions`, plus the app-shell specs) |
...
### 🟡 F13. `VendorAdminService` and its consuming components have no test coverage
Found in Slice 2's final whole-branch review (2026-08-04). Task 3's `legalVendorActions()` spec is the only new test code the whole slice added — `VendorAdminService` itself (the `refreshAsync`/`getVendorAsync`/`decide` async siblings, `refreshRequestId` sequencing, `adjustServerStats`), `vendor-queue.component.ts`, and `vendor-review.page.ts` are exercised only by the build and by one live manual pass against the running API, not by any unit test. The mock-mode branches — `decide()`'s `mockCall()` path, `transition()` — have no automated coverage at all, which is exactly the surface most sessions actually run on today (`useRealApi` is committed `false`).
