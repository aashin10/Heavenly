...
| Services portal | **Auth, profile editing, and the request wizard wired to the real API** (F2.1–F2.3, behind `useRealApi`, committed `false`). Tender/bid/award screens still on `localStorage`. Backend: **B2 COMPLETE** — request → tender → bid → award runs end to end |
...
- Remaining under F2: tender browse, bid submission, admin verification/evaluation screens, award flows. None started yet.
