...
**Last updated:** 2026-08-06 · **Maintained on the go** — updated after every coding iteration, not retroactively.
...
| Admin surface | **Unblocked (B11) and in use (F2.5).** A `ServiceAdmin`/`Admin` account can exist, be granted, and now actively verifies vendors through `/management` — every `/api/service-admin/vendors` route live-verified |
| Dependencies | **0 vulnerable packages** (verified 2026-07-26) |
| Tests | **215 backend integration tests**, Testcontainers-backed, self-contained. **34 frontend specs** (interceptor, token-refresh coordinator, `legalVendorActions`, `VendorAdminService` mock transitions, the vendor queue's conditional columns, the review page's reason panel, plus the app-shell specs) |
...
### 🔴 F16. A `ServiceAdmin` cannot open `/management` at all — the role Slice 1 exists to grant is refused by the frontend
Found in Slice 2's post-merge review (2026-08-06). `/management` is gated by `adminGuard` → `AuthService.isAdmin()`, which is `userType === 'admin'` (`auth.service.ts:28`), against a `UserType` union of `'employer' | 'applicant' | 'admin'` (`user.model.ts:1`). The backend authorizes these routes as `[Authorize(Roles = "ServiceAdmin,Admin")]` and emits `userType: "service_admin"` for such an account (`LoginUserCommandHandler.cs:88`). So a genuine `ServiceAdmin` is bounced with *"Access denied. Admin privileges required."* and redirected to `/dashboard`; the navbar also hides the Management link for them (`navbar.component.ts:105`). Only the seeded platform `Admin` can reach the screen.

This makes B11's grant/revoke endpoint — the entire deliverable of Slice 1 — decorative for its primary purpose: you can grant `ServiceAdmin`, and the grantee still cannot verify a vendor through the UI. The inverse gap exists too: `GetRoleNamesForClaims` only falls back to `UserType` when `Roles` is empty, so a `UserType.Admin` whose `user_roles` say otherwise passes the guard and then 403s on every request.

`AuthResponse.roles[]` is already returned by the server and already modelled on the client (`auth-api.models.ts:19,33`) — and is read nowhere in `AuthService` (verified: zero occurrences). The fix is to persist it and make `isAdmin` role-based, mirroring the controller's own `Roles=` string rather than the primary `userType`:
```ts
...
```
**Deliberately not fixed in that review.** It is an auth-path change that cannot be honestly verified without a running stack and a real `ServiceAdmin` login, and shipping an unverified auth change is exactly what the verification discipline forbids. It also overlaps F3 (collapse the two auth stores), which is where the durable fix belongs — `ServiceAuthService` already has a correct `isServiceAdmin()` (`service-auth.service.ts:68`); the defect is that `/management` consults the *jobs* store. Fix deliberately, with a live pass, before Slices 4+ depend on delegated admins.
→ related: [F3](#-f3-collapse-the-two-auth-stores-c1) · B11
